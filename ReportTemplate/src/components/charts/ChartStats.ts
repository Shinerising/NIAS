...
export default (
  ...
  const list = (switchList as { State: number[] }[])?.concat(
    (hostList as { State: number[] }[]) ?? { State: [] }
  );
  const stateCount: [number, number, number, number, number] = list?.reduce(
    (prev, curr) =>
      curr.State.reduce(
        (_prev, _curr) => {
          _prev[_curr] += 1;
  ...
  const inSpeedCount: [number, number, number, number] = hostList?.reduce(
    (prev, curr) =>
      curr.InSpeed.reduce(
        (_prev, _curr) => {
          let idx = 0;
  ...
  const outSpeedCount: [number, number, number, number] = hostList?.reduce(
    (prev, curr) =>
  ...
  const speedCount = inSpeedCount.map((v, i) => v + outSpeedCount[i]);
  const latencyCount: [number, number, number, number] = hostList?.reduce(
    (prev, curr) =>
      curr.Latency.reduce(
        (_prev, _curr) => {
          let idx = 0;
