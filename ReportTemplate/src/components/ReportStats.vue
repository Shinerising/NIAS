...
const getStats = () => {
  const [healthStats, networkStats, SensorStats, PortStats] = props.data.Stats ?? [];
  const healthScore = caculatePercent(healthStats);
  const healthInfo =
    healthStats === undefined ? "暂无数据" :
      healthScore === 1 ? "在过去24小时内，网络运行过程基本平稳。" :
        `在过去24小时内，局域网内部发生通信障碍，中断时长共计${healthStats[0] + healthStats[2]}分钟。建议查看日志记录信息了解故障发生时间和发生位置，及时进行维护和优化工作。`;
  const networkScore = caculatePercent(networkStats);
  const networkInfo =
    networkStats === undefined ? "暂无数据" :
      networkScore === 1 ? "在过去24小时内，网络通信质量基本平稳。" :
        `在过去24小时内，局域网内部${networkStats[0] + networkStats[2]}分钟出现短时流量大于正常值，${networkStats[4]}分钟出现短时网络延迟大于正常值。建议查看日志记录信息了解详细信息，及时检测设备是否存在网络工作障碍。`;
  const SensorScore = caculatePercent(SensorStats);
  const SensorInfo =
    SensorStats === undefined ? "暂无数据" :
      SensorScore === 1 ? "在过去24小时内，所有设备运行基本平稳。" :
  ...
  const PortInfo =
    PortStats === undefined ? "暂无数据" :
      PortScore === 1 ? "在过去24小时内，暂未发现网络安全风险。" :
...
const deviceList = {
  ...
  hostList: props.data.DeviceInfo?.map((item) => ({
    ...
    address: item.Address,
    mac: item.MACAddress,
    brief: [item.OS, item.Vendor].filter(item => item !== undefined && item !== "").join(" , "),
  })),
};
</script>
...
<template>
  ...
  <div class="record-box">
    <p class="text">
      <b>安全风险指数：</b>
      <span>描述网络潜在安全风险，与设备类型、操作系统版本与高风险端口相关。</span>
    </p>
    <p class="text">
      {{ stats.PortInfo }}
    </p>
